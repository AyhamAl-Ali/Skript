...
public class ChatMessages {
	...
	static final Pattern linkPattern = Pattern
			.compile("[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zA-Z0-9@:%_\\+.~#?&//=]*)");
	...
	/**
	 * Registers language change listener for chat system. Called once by Skript,
	 * please don't call this addon developers.
	 */
	...
	static {
		Gson nullableGson = new GsonBuilder()
				.registerTypeAdapter(boolean.class, new MessageComponent.BooleanSerializer()).create();
		...
		for (int i = 0; i < chars.length; i++) {
			...
			if (c == '<') { // Tag parsing
				...
				if (end != -1) { // If this COULD be valid tag...
					...
					// If this is invalid tag, just ignore it. Maybe scripter was trying to be
					// clever with formatting
				}
			} else if (c == '&' || c == '§') {
	 ...
	 * additional styling, i.e. if text was not bold and is bold, it will remain
	 * bold.
	 * 
	 * @param from
	 * @param to
	...
	public static void copyStyles(MessageComponent from, MessageComponent to) {
		...
		// Links and such are never reset by color codes - weird, but it'd break too
		// much stuff
	 ...
	 * 
	 * @param code Something that implements {@link ChatCode}. For inspiration,
	 *             check {@link SkriptChatCode} source code.
	 */
	...
	public static String stripStyles(String text) {
		...
		/*
		 * 1st Replace: strips json 2nd Replace: strips colors using <color name> (Note:
		 * this regex expr repalce any two words surrounded by <> such as <any text>)
		 * 3rd Replace: strips colors & or § 4th Replace: strips colors using hex codes
		 */
		plain = plain.replaceAll("<(link|url|run command|suggest command|tooltip):(.*?)>", "$2")
				.replaceAll("<\\w+? ?\\w+?>", "").replaceAll("(?i)[&§][0-9a-folkrnm]", "");
