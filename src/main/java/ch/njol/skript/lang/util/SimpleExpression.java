...
public abstract class SimpleExpression<T> implements Expression<T> {
	...
	@Override
	public final boolean check(final Event event, final Checker<? super T> c) {
		return check(event, c, false);
	}
	
	@Override
	public final boolean check(final Event event, final Checker<? super T> c, final boolean negated) {
		return check(get(event), c, negated, getAnd());
	}
