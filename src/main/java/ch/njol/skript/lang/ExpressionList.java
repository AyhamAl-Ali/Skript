...
public class ExpressionList<T> implements Expression<T> {
	...
	@Override
	public boolean check(Event event, Checker<? super T> c, boolean negated) {
		return negated ^ check(event, c);
	}

	@Override
	public boolean check(Event event, Checker<? super T> c) {
		for (Expression<? extends T> expr : expressions) {
			boolean b = expr.check(event, c);
			if (and && !b)
				return false;
			if (!and && b)
				return true;
